...
def get_velocity_form(
    ...
    nhat = model.nhat
    gamma = model.constants['gamma']
    omega = model.constants['omega']
    alpha = model.constants['alpha']
    delta = model.constants['delta']
    eps_reg = model.constants['eps_reg']
    n = model.constants['n']
...
class CoupledModel:
    def __init__(
        ...
        # This ensures non-singularity when SSA simplification is used.  self.area 
        if ssa:
